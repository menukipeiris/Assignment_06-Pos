import { items, order_details_db, orders} from "../db/db.js";
...
order_search.on('input', function (){
    ...
    let matchingOrders = orders.filter(order => order[option].toLowerCase() === searchTerm);
    ...
    if (index >= 0){
        ...
        for(let i=0; i<order_details_db.length; i++){
            if (orderId === order_details_db[i].order_id){
                ...
                let item_index = items.findIndex(item => item.item_code === order_details_db[i].item_id);
